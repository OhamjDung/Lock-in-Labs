...
def main():
    st.title("🧬 Lock In Labs: Life Architect")
    st.caption("v1.0.0 - Prototype Interface")

    data = load_data(DATA_FILE)

    if not data:
        st.warning("⚠️ No Character Sheet found.")
    ...
    with col1:
        st.metric("💼 Career", f"Lvl {c_score}", delta=f"{len(sheet.get('stats_career', {}))} Stats")
        with st.expander("Details"):
            st.json(sheet.get("stats_career", {}))
    with col2:
        st.metric("💪 Physical", f"Lvl {p_score}", delta=f"{len(sheet.get('stats_physical', {}))} Stats")
        with st.expander("Details"):
            st.json(sheet.get("stats_physical", {}))
    with col3:
        st.metric("🧠 Mental", f"Lvl {m_score}", delta=f"{len(sheet.get('stats_mental', {}))} Stats")
        with st.expander("Details"):
            st.json(sheet.get("stats_mental", {}))
    with col4:
        st.metric("❤️ Social", f"Lvl {s_score}", delta=f"{len(sheet.get('stats_social', {}))} Stats")
        with st.expander("Details"):
    ...
    st.subheader("🕸️ The Neural Interface (Skill Graph)")
    ...
    if nodes:
        ...
        st.info("💡 **Legend:** Octagon = Goal | Box = Sub-Skill | Oval = Daily Habit. Colors match the Pillars above.")
    ...
    if habits:
        ...
        for i, habit in enumerate(habits):
            ...
            with col:
                ...
                if checked:
                    st.toast(f"✅ Completed: {habit['name']}! Gained XP.")
    else:
